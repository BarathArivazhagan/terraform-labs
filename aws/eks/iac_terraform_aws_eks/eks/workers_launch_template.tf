...
resource "aws_autoscaling_group" "workers_launch_template" {
  ...
  target_group_arns = [
    compact(split(",", coalesce(lookup(var.worker_groups_launch_template[count.index], "target_group_arns", ""), local.workers_group_launch_template_defaults["target_group_arns"])))]
  ...
  tags = [
    concat(
    list(
      map("key", "Name", "value", "${aws_eks_cluster.eks_cluster.name}-${lookup(var.worker_groups_launch_template[count.index], "name", count.index)}-eks_asg", "propagate_at_launch", true),
