...
resource "aws_autoscaling_group" "workers" {
  ...
  vpc_zone_identifier   = [
    split(",", coalesce(lookup(var.worker_groups[count.index], "subnets", ""), local.workers_group_defaults["subnets"]))]
  ...
  suspended_processes   = [
    compact(split(",", coalesce(lookup(var.worker_groups[count.index], "suspended_processes", ""), local.workers_group_defaults["suspended_processes"])))]
  enabled_metrics       = [
    compact(split(",", coalesce(lookup(var.worker_groups[count.index], "enabled_metrics", ""), local.workers_group_defaults["enabled_metrics"])))]
  ...
  tags = [
    concat(
    list(
      map("key", "Name", "value", "${aws_eks_cluster.eks_cluster.name}-${lookup(var.worker_groups[count.index], "name", count.index)}-eks_asg", "propagate_at_launch", true),
